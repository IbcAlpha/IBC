...
class TwsSettingsSaver {
    ...
    private static List<Date> convertSuppliedTimes(String[] times) throws TwsAutomaterException {
    ...
    private static List<Date> generateSaveTimes(String[] times) throws TwsAutomaterException {
        int interval = 0;
        try {
            interval = Integer.parseInt(times[1]);
        } catch (NumberFormatException e) {
            throw new TwsAutomaterException("interval is '" + times[1] + "' but should be an integer");
        }
        ...
        if (interval < 1 || interval > 1439) {
            throw new TwsAutomaterException("the saving interval must be between 1 and 1439 minutes");
        }
    ...
    private static Calendar getCalendarForTime(String time) throws TwsAutomaterException {
        Calendar cal = Calendar.getInstance();
        try {
            cal.setTime(dateFormat.parse(time));
        } catch (ParseException e) {
            throw new TwsAutomaterException("value '" + time + "' should be in hh:mm format");
        }
