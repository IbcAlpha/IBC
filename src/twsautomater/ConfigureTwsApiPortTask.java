...
class ConfigureTwsApiPortTask implements ConfigurationAction{
    ...
    public void run() {
        try {
            ...
            if (comp == null) throw new TwsAutomaterException("could not find socket port component");

            JTextField tf = SwingUtils.findTextField((Container)comp, 0);
            if (tf == null) throw new TwsAutomaterException("could not find socket port field");
            ...
            if (currentPort == portNumber) {
                ...
                if (!MainWindowManager.mainWindowManager().isGateway()) {
                    ...
                    if (cb == null) throw new TwsAutomaterException("could not find Enable ActiveX checkbox");
