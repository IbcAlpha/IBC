...
final class GatewayLoginFrameHandler extends AbstractLoginHandler {
    ...
    @Override
    protected final boolean initialise(final Window window, int eventID) throws TwsAutomaterException {
    ...
    @Override
    protected final boolean preLogin(final Window window, int eventID) throws TwsAutomaterException {
        boolean result;
    ...
    @Override
    protected final boolean setFields(Window window, int eventID) throws TwsAutomaterException {
        if (Settings.settings().getBoolean("FIX", false)) {
    ...
    private void selectGatewayMode(Window window) throws TwsAutomaterException {
        if (Settings.settings().getBoolean("FIX", false)) {
            switchToFIX(window);
        } else {
    ...
    private void switchToFIX(Window window) throws TwsAutomaterException {
        JRadioButton button = SwingUtils.findRadioButton(window, "FIX CTCI");
        if (button == null) throw new TwsAutomaterException("FIX CTCI radio button");
        
        if (! button.isSelected()) button.doClick();
    }
    
    private void switchToIBAPI(Window window) throws TwsAutomaterException {
        ...
        if (button == null) throw new TwsAutomaterException("IB API radio button");
