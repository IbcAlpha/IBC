...
class Utils {
    ...
    static boolean invokeMenuItem(final Container container, final String[] path) throws IllegalStateException {
        ...
        while (true) {
            FutureTask<Boolean> task = new FutureTask<>(new Callable<Boolean>() {
                @Override
                public Boolean call() throws TwsAutomaterException {
                    String s = path[0];
                    for (int i = 1; i < path.length; i++) s = s + " > " + path[i];

                    JMenuItem menuItem = SwingUtils.findMenuItemInAnyMenuBar(container, path);
                    if (menuItem == null) throw new TwsAutomaterException("menu item: " + s);
            ...
            try {
                ...
                if (t instanceof TwsAutomaterException) {
                    return false;
                }
    ...
    private static String formatMessage(String message) {
        return _DateFormatter.format(new Date()) + " TwsAutomater: " + message.substring(0,1).toUpperCase() + message.substring(1);
    }
     ...
     * @throws TwsAutomaterException
     * a UI component could not be found
     * @throws IllegalStateException
     * the method has not been called on the SWing event dispatch thread
     */
    static boolean selectConfigSection(final JDialog configDialog, final String[] path) throws TwsAutomaterException, IllegalStateException {
        ...
        if (configTree == null) throw new TwsAutomaterException("could not find the config tree in the Global Configuration dialog");
